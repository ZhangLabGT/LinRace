...
#' @import phangorn
#' @export
NJ_asym_Dif <- function(muts, states, counts, state_lineages, max_Iter = 200,lambda1 = 10,lambda2 = 1) {
  labels <- rownames(muts)
  X <- muts
  #X <- BarToMut(muts)
...
#' Finding the best tree structure based on local search/ Fast Newick implementation
#'
#' @param states the states of the gene expressions of cells
#' @param labels the labels of cells
#' @param state_lineages the lineages that makes the state network from the root state to leaf states
...
#' @param lambda2 hyperparameter for neighbor distance likelihood
#' @import castor
FindExpTree_Dif_Newick <- function(states, labels, state_lineages, muts, prob_f, newick_lookup = NULL, maxIter = 500, lambda1, lambda2) {
  #browser()
  leafs <- c()
  for (i in 1:length(labels)) {
    label <- labels[i]

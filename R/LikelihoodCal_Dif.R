...
#' @import destiny
#' @export
LikelihoodCal_Dif <- function(tree,muts,prob_t,cell_state_labels,state_lineages,newick_lookup){
  #browser()
  N_char <- dim(muts)[2]
  ...
  if (max_step > 0){
    ...
    l_expression <- sum(state_transitions$freq * transit_prob) - N_violations*50 + 10 * ad_loss + diffuse_loss
  }else {
    l_expression <- - N_violations*50 + 10 * ad_loss + diffuse_loss
  }
  #browser()

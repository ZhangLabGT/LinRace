...
```{r}
rf_linrace <- data.frame(ncells = numeric(),mu = numeric(),pd = numeric(),run = numeric(),rf = numeric())
for (mu in c(0.05,0.1,0.2)){
  for (pd in 0){
    for (ncells in c(1024)){
      for (i in c(1,3,4,5,6,7,8,10)){
        tree_recon_dir <- sprintf("~/LinRace/results/%g_mu_%g_pd_%g_run_%g_bin_tree.newick", 
                          ncells, mu,pd, i)
        tree_gt_dir <- sprintf("~/LinRace/tree_%g.tree", 
                          ncells)
        tree_linrace <- read.tree(file = tree_recon_dir)
        #tree_lintimat <- drop.tip(tree_lintimat,'normal')
        tree_gt <- read.tree(file = tree_gt_dir)
        tip_label <- c()
        for (node in tree_gt$tip.label){
        ...
        print( RF.dist(tree_linrace, tree_gt, normalize = TRUE))
        temp <- data.frame(ncells = ncells,mu = mu,pd = pd,run = i,rf = RF.dist(tree_linrace, tree_gt, normalize = TRUE))
        rf_linrace <- rbind(rf_linrace,temp)
      }
    }
}
...
character_matrix_dir <- sprintf("~/LinRace-temp/datasets/mut_%g_mu_%g_pd_%g_run_%g.csv", 
                               ncells, mu, p_d,i)
meta_dir <- sprintf("~/LinRace-temp/datasets/meta_%g_mu_%g_pd_%g_run_%g.csv", 
                               ncells, mu, p_d,i)

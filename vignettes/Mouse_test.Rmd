...
#loading 10X objects
```{r}
fpath_2a <- system.file("extdata", "2a/", package="LinRace")
fpath_2b <- system.file("extdata", "2b/", package="LinRace")

embryo2a.data <- Read10X(data.dir = fpath_2a,gene.column = 1)
embryo2b.data <- Read10X(data.dir = fpath_2b,gene.column = 1)
```
...
fpath_meta_m <- system.file("extdata", "GSE122187_CellStateKernels.xls", package="LinRace")
fpath_meta_ct <- system.file("extdata", "cell_states_me.txt", package="LinRace")

cluster_centers <- read_excel(fpath_meta_m)
ct_mouse <- read.table(fpath_meta_ct,sep = "-")
...
```{r}
fpath_cm <- system.file("extdata", "test_embryo2_character_matrix.txt", package="LinRace")
cm_embryo2 <- read.csv(fpath_cm,sep = "\t")
...
#Running Slingshot
```{r}
rownames(umap_res) <- paste("cell",1:(nrow(muts_embryo2)),sep = "_")
labels_new <-as.character(embryo2_sub$cluster.idents)
...
```{r}
fpath_tree <- system.file("extdata", "tree_linrace_embryo2.newick", package="LinRace")
tree_linrace <- read.tree(fpath_tree)

...
```{r}
# Run LinRace
returnList <- Preprocessing(muts_leaves,phyla,states_leaves)
...
counts <- t(true_counts_res[[1]])[order,]
...
tree <- NJ_asym_Dif(as.data.frame(muts_leaves),cell_meta$cluster_kmeans,counts,state_lineages,max_Iter = 500, lambda1 = 10,lambda2 = 1)

plot(tree)
```

Two baseline neighbor joining on either barcode data or gene expression data
...
#extensive analysis on different local search programs with the likelihood function
```{r}
...
for(ncells in ncells_list){
  for(mu in mu_list){
    for(i in 1:5){
      ...
      # write data for LinTiMat
      counts <- true_counts_res[[1]]
      exdata <- preprocessSC(counts)
      sf <- apply(exdata, 2, mean)
      npX <- t(t(exdata) / sf )
      lnpX <- log(npX+1)
      lnpX_imp <- DrImpute(lnpX,seed = seeds[i])
      ...
      for (k in 1:ncells){
        ...
        temp_df <- data.frame(cell_id=cell_id,ClusterIdent = states[i,2], HMID = barcode,
                              stringsAsFactors = F)
        profiles <- rbind(profiles,temp_df)
      }
  
      profiles_out <- cbind(profiles,counts_t)
      #profiles_out <- cbind(profiles[1:376,],counts_Lin)
      ...
      character_matrix_dir <- sprintf("~/LinRace-temp/datasets/LinRace_compare/cm_tedsim_mu%g_p_d%g_%g_%g.txt", 
                               mu, p_d, ncells,i)
          
      counts_dir <- sprintf("~/LinRace-temp/datasets/LinRace_compare/counts_barcoded_tedsim_mu%g_p_d%g_%g_%g.txt", 
                           mu, p_d,ncells, i)
      meta_dir <- sprintf("~/LinRace-temp/datasets/LinRace_compare/Top_genes_tedsim_mu%g_p_d%g_%g_%g.txt", 
                         mu, p_d,ncells,  i)
      write.table(muts_leaves,character_matrix_dir,sep = "\t", quote = FALSE)
      write.table(profiles_out,file = counts_dir,row.names = FALSE,sep = "\t", quote = FALSE)
      write.table(subset(rank_gene,select = 1),file = meta_dir,row.names = FALSE,sep = "\t", quote = FALSE)
    }
  }
}
...
#df_plot$ncells <- c(rep(64,15),rep(256,15),rep(1024,15))
df_plot$ncells <- df_plot$run
df_plot$ncells_group <- as.factor(df_plot$ncells)

for (ncells in ncells_list){
  p <- ggplot(df_plot[df_plot$ncells == ncells,], aes(x=mu_group, y=rf, fill=method)) + 
    geom_boxplot()
  ...
  plt_dir <- sprintf("~/LinRace-temp/plots/box_compare_ncells%g.png",ncells)
  ggsave(plt_dir,plot = p)

}

for (mu in mu_list){
  p <- ggplot(df_plot[df_plot$mu == mu,], aes(x=ncells_group, y=rf, fill=method)) + 
    geom_boxplot()
  p <- p + theme(axis.text = element_text(size = 20), axis.title = element_text(size = 30),legend.title = element_text(size = 15),legend.text = element_text(size = 20))
  plt_dir <- sprintf("~/LinRace-temp/plots/box_compare_mu%g.png",mu)
  ggsave(plt_dir,plot = p)

}

p + theme(axis.text = element_text(size = 20), axis.title = element_text(size = 30),legend.title = element_text(size = 15),legend.text = element_text(size = 20))

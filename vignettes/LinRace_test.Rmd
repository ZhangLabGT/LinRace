...
df_plot$ncells <- c(rep(64,15),rep(256,15),rep(1024,10))
df_plot$ncells_group <- as.factor(df_plot$ncells)


p <- ggplot(df_plot[df_plot$ncells == 64,], aes(x=mu_group, y=rf, fill=method)) + 
    geom_boxplot()
...
p <- ggplot(df_plot[df_plot$mu == 0.05,], aes(x=ncells_group, y=rf, fill=method)) + 
    geom_boxplot()
p + theme(axis.text = element_text(size = 20), axis.title = element_text(size = 30),legend.title = element_text(size = 15),legend.text = element_text(size = 20))
